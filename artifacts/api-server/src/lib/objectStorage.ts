...
function buildGcsCredentials() {
  const encoded = process.env.GCS_SERVICE_ACCOUNT_B64;
  if (!encoded) return undefined;
  const json = Buffer.from(encoded, "base64").toString("utf8");
  return JSON.parse(json);
}

const gcsCredentials = buildGcsCredentials();

export const objectStorageClient = new Storage(
  gcsCredentials
    ? { credentials: gcsCredentials, projectId: process.env.GCS_PROJECT_ID || undefined }
...
export class ObjectStorageService {
  ...
  async deleteObjectEntity(rawPath: string): Promise<boolean> {
    const normalizedPath = this.normalizeObjectEntityPath(rawPath);
    if (!normalizedPath.startsWith("/objects/")) return false;
    try {
      const objectFile = await this.getObjectEntityFile(normalizedPath);
